/*
 * CONFIRMED OPERATIONAL: 2017-02-08
...
//for another 64 bytes. (64 bytes is default Arduino serial buffer)
...
void request_data_if_ready(void){
                                //(loadpoint - readpoint) == number of buffer addresses full.
                                //Total number of buffer addresses is 256, and we take data in 64-byte blocks.
                                //Our buffer addresses each require two bytes to fill,
                                //so if (loadpoint - readpoint) < 225, we have at least (256 - 224) == 32 addresses to fill.
                                //32 addresses at 16 bits each == 64 bytes, which means if we don't already have data to load,
                                //we can safely request another 64-byte block to put into our buffer.
                                
  if(((loadpoint - readpoint) < 225)    //If we have space for more data in the buffer,
    && (Serial.available() <= 1)){      //and we don't already have data to load that's currently in the queue,
...
void load_data_if_available(void){      //Loads data, as long as there is data to load. 
  ...
  while(Serial.available() > 1){        //As long as there are two or more bytes to load,
